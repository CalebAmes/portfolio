export const fetchMessages = async (id: number = 1) => {
	const data: any = await fetch(`https://shrewdness.herokuapp.com/api/channelMessages/${id}`)
		.then((res) => res.json())
...
export const fetchPokemon = async (offset: number = 0, limit: number = 25) => {
	const data: any = await fetch(`https://pokeapi.co/api/v2/pokemon/?offset=${offset}&limit=${limit}`)
		.then((res) => res.json())
		.catch((err) => console.error(err));
	return data.results.map((el: PokemonApi, idx: number) => dataFormatter(el, idx + offset + 1));
};
